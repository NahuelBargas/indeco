...
basezonas<-read.xlsx("bases/basezonas50.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_octubre2024.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("107.0 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_11_24.xls")%>%		 
...
valueBox("-0.3 %", caption = "Variación del EMAE desestacionalizado de Sept-24 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros diez meses de 2024",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2024/11/sector_publico_base_caja_octubre_2024.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Octubre 24. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Octubre 2024 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
