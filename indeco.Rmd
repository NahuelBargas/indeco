...
trimestre<-seq(as.Date("2016-04-01"), length = 21,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 70)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 71, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 70)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas19.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_diciembre21.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("50.9 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_01_22.xls")%>%
...
#polarcolumn
 aperturas_graf<- map(unique(aperturas$Aperturas), function(niveles){
  ...
  hc_credits(enabled = TRUE, text = "Fuente: INDEC",align = "right",verticalAlign = "bottom",
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_12_21.xls")%>%
  hc_exporting(enabled = TRUE)
})  
```
...
valueBox("-0.8 %", caption = "Descenso del EMAE desestacionalizado de Octubre-21 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("10.4 %", caption = "Variación acumulada del año(2021) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, tercer trimestre del 2021",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 569629 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
hc_exporting(enabled = TRUE)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Tercer trimestre 2021. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
hc_exporting(enabled = TRUE)
...
### Balanza comercial, doce meses del 2021
...
impo <- tibble(
  ...
  y = c(63184,10181,25758,5843,11890,7236,1692,584
...
expo <- tibble(
  ...
  y = c(77934,21828,30951,19940,5215
...
y = c(24,1722,220,670,628,13659,3885,244,47,185,397,146
...
name = c("Carnes y sus preparados",
  ...
  "Lanas elaboradas",
  "Resto de MOA"),
y=c(3505,249,1144,67,133,159,847,8700,305,783,974,12746,273,451,128,486
...
name= c(  "Productos químicos y conexos",
  ...
  "Resto de MOI"),
y= c(5013,1042,233,29,458,123,9,154,2561,1843,1377,6475,162,462
...
y=c(2931,1329,10341,3183,913,1025,572,3871,1593
...
y=c(250,572,710,2253,536,330,452,527,449,259,471,426
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado doce meses del 2021",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 25/1/2022.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2022/01/resultado_fiscal_diciembre_2021-1.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Diciembre 2021.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Diciembre 2021- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
