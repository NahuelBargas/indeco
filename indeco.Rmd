...
producto$Fecha <- as.yearqtr(2004 + seq(0, 72)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 73, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 72)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas24.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado-fiscal_mayo_2022.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
hc_subtitle(text="El VAB total asciende a 571371 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Primer trimestre 2022. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
### Balanza comercial, primeros cinco meses de 2022
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros cinco meses de 2022",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 27/6/2022.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2022/06/may-22.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Mayo 2022.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Mayo 2022- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
