...
producto$Fecha <- as.yearqtr(2004 + seq(0, 79)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 80, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 79)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas44.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_febrero2024.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("65.0 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_05_24.xls")%>%		 
...
valueBox("-0.2 %", caption = "Variación del EMAE desestacionalizado de Febrero-24 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("-3.6 %", caption = "Variación acumulada del año(2024) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, cuarto trimestre del 2023",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 587548 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Cuarto trimestre 2023. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primer trimestre de 2024",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 18/5/2024.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2024/05/sector_publico_base_caja_abril_2024.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Abril 24.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Abril 2024- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
