...
basezonas<-read.xlsx("bases/basezonas34.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_marzo_2023.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("21.7 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_04_23.xls")%>%		 
...
#polarcolumn
 aperturas_graf<- map(unique(aperturas$Aperturas), function(niveles){
  ...
  hc_credits(enabled = TRUE, text = "Fuente: INDEC",align = "right",verticalAlign = "bottom",
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_03_23.xls")%>%
    hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
})  
```
...
valueBox("0.0 %", caption = "Variación del EMAE desestacionalizado de Febrero-23 con respecto al mes anterior",color="skyblue")
...
valueBox("1.6 %", caption = "Variación acumulada del año(2023) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="orangered")
...
### Balanza comercial, primer trimestre de 2023
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primer trimestre de 2023",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2023/04/sector_publico_base_caja_-_marzo_23.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Marzo 2023.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Marzo 2023- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
