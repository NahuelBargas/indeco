...
trimestre<-seq(as.Date("2016-04-01"), length = 31,by = "3 months")
...
basezonas<-read.xlsx("bases/basezonas46.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_mayo2024.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("71.9 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_06_24.xls")%>%		 
...
valueBox("-0.1 %", caption = "Variación del EMAE desestacionalizado de Abril-24 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("-4.2 %", caption = "Variación acumulada del año(2024) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros cinco meses de 2024",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 21/6/2024.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2024/06/sector_publico_base_caja_mayo_2024.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Mayo 24.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Mayo 2024- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
