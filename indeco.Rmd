...
trimestre<-seq(as.Date("2016-04-01"), length = 36,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 85)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 86, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 85)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas56.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_septiembre25.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("22.0 %", caption = "Acumulada en lo que va del año",  color="skyblue")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_10_25.xls")%>%		 
...
valueBox("-0.1 %", caption = "Variación del EMAE desestacionalizado de Jul-25 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("5.6 %", caption = "Variación acumulada del año(2025) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="#43ac6a")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, primer trimestre del 2025",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 646727.30 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Segundo trimestre 2025. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado ocho meses de 2025",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2025/10/sector_publico_base_caja_septiembre_2025_1.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Septiembre 2025. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Septiembre 2025 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
