...
trimestre<-seq(as.Date("2016-04-01"), length = 40,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 88)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 89, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 88)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas67.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_julio26.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
basecaja<-basecaja[,c(1,2,5)]
names(basecaja)<-c("Concepto","Monto", "Var Anual")

basecaja<- basecaja %>%
             mutate_at(c("Monto", "Var Anual"),as.numeric)%>%
             mutate_at(c("Monto", "Var Anual"),round,2)
...
regiones<-read.xlsx("bases/opex 1er semestre 2026.xlsx",sheet=1)

provincias<-jsonlite::fromJSON("bases/provincias.geojson", simplifyVector = FALSE)

complejos<-read.xlsx("bases/opex 1er semestre 2026.xlsx",sheet=2)
...
```{r}
valueBox(paste(format(last(ipcmensual$IPC.General)),"%"), caption = paste0("Variación mensual del IPC - ",str_to_title(format(max(ipcmensual$Período),"%B-%Y"))), icon="glyphicon-arrow-up",color="skyblue")
```
...
```{r}
valueBox("19.3 %", caption = "Acumulada en lo que va del año",  color="skyblue")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = "https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_08_26.xls")%>%		 
...
labs(x="Trimestres",y="Coeficientes estimados (%)",colour="",linetype="",caption = "En base a datos obtenidos de la EPH-INDEC(www.indec.gob.ar)")+
theme_classic()+
...
hc_xAxis(type = "category",title=list(text = "Trimestres"))%>%
  hc_yAxis(title=list(text = "Porcentaje %"))%>% 
   ...
   {point.y:.2f}% ")%>%
      hc_plotOptions(series=list(states=list(inactive=list(opacity= 0.6))))%>%
    hc_subtitle(text = "En base a datos obtenidos de la EPH-INDEC (www.indec.gob.ar) - Estimaciones vía Bootstrap de mil repeticiones.",
              align = "right",verticalAlign = "bottom", style = list(color = "#2b908f", fontSize = '10px'))%>%
    hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
```

### Tasa de incidencia de la pobreza e indigencia- 2ndo. Semestre 2025- Aglomerados
...
### Pobreza e indigencia-2ndo. semestre 2025-Regiones
...
geom_vline(xintercept=producto$fecha1[68],color="grey",size=0.3,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[72],color="grey",size=0.3,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[76],color="grey",size=0.3,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[80],color="grey",size=0.3,linetype="dashed")+
scale_x_date(name="Trimestres",breaks=producto$fecha1,labels=producto$Trimestre,sec.axis = sec_axis(~.,labels = scales::date_format("%Y"),breaks=seq(as.Date("2004-06-01"), length = 21, by = "12 months")))+
theme_classic()+
...
valueBox("0.8 %", caption = "Variación del EMAE desestacionalizado de Jun.-26 con respecto al mes anterior", icon="glyphicon-arrow-up",color="skyblue")
...
valueBox("1.9 %", caption = "Variación acumulada del año(2026) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="#43ac6a")#"color="yellow")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, primer trimestre del 2026",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 599,984 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Primer trimestre 2026. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)

```

Balanza comercial, acumulado en 2026 {data-navmenu="Comercio Exterior"}
=========================================================================


### Balanza comercial, acumulado 2026
...
impo <- tibble(
  ...
  y = c(import[1,2],import[2,2],import[6,2],import[12,2],import[15,2],import[19,2],import[27,2],import[28,2]
...
bk<- tibble(
name= c("Bienes de capital excluido equipos de transporte", 
"Computadoras y Telefonos",
"Equipos de transporte industriales"),
y=c(import[3:5,2]
...
bi<-tibble(
name=c("Alimentos y bebidas basicos fundamentalmente para la industria", 
"Alimentos y bebidas elaborados fundalmentalmente para la industria",
"Suministros industriales basicos no especificados en otras partidas",
"Suministros industriales elaborados no especificados en otras partidas",
"Medicamentos"),
y=c(import[7:11,2]
...
cyl<- tibble(
name=c("Combustibles y lubricantes basicos",
...
pya<- tibble(
name=c("Piezas y accesorios para bienes de capital",
"Partes y piezas para computadoras y telefonos",
"Piezas y accesorios para equipos de transporte"),
y=c(import[16:18,2]

))

bco<-tibble(
name=c("Alimentos y bebidas basicos fundalmentalmente para el hogar", 
"Alimentos y bebidas elaborados fundalmentalmente para el hogar", 
"Equipos de transporte no industriales", 
"Articulos de consumo duraderos no especificados en otra partida", 
"Articulos de consumo semiduraderos no especificados en otra partida", 
"Articulos de consumo no duraderos no especificados en otra partida",  
"Medicamentos"),
y=c(import[20:26,2]
...
malasia<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[19,2],saldozonas[19,3]))

tailandia<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[20,2],saldozonas[20,3]))

vietnam<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[21,2],saldozonas[21,3]))

medio<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[22,2],saldozonas[22,3]))

magreb<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[23,2],saldozonas[23,3]))

sacu<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[24,2],saldozonas[24,3]))

oceania<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[25,2],saldozonas[25,3]))

restom<-tibble(
name=c("Exportaciones","Importaciones"),
y=c(saldozonas[26,2],saldozonas[26,3]))
...
dbrasil<-list_parse(brasil)
dperu<-list_parse(peru)
dusa<-list_parse(usa)
dvietnam<-list_parse(vietnam)
dindonesia<-list_parse(indonesia)
dparaguay<-list_parse(paraguay)
duruguay<-list_parse(uruguay)
dtailandia<-list_parse(tailandia)
dmalasia<-list_parse(malasia)
dalemania<-list_parse(alemania)
despaña<-list_parse(españa)
ditalia<-list_parse(italia)
dpaisesbajos<-list_parse(paisesbajos)
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros siete meses de 2026",style = list( fontSize = '12px')) %>%
hc_subtitle(text="INDEC - Datos Estimados. Millones de dólares. Cliquear para desplegar.",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)

```

Saldo y comercio exterior total, por zona zona económica - 2026 {data-navmenu="Comercio Exterior"}
=================================================================================
...
function (tm, ...) {
  ...
  ds <- purrr::map_dfr(seq(ndepth), function(lvl) {

    lvl_name <- names(df)[lvl]

    df2 <- df %>%
      dplyr::filter(level == lvl) %>%
      dplyr::rename(name = !!lvl_name) %>%
      dplyr::mutate(id = highcharter::str_to_id(name))

    if (lvl > 1) {
      parent_name <- names(df)[lvl - 1]
      df2 <- df2 %>%
        dplyr::mutate(
          parent = !!rlang::sym(parent_name),
          parent = highcharter::str_to_id(parent)
        )
    } else {
      df2 <- df2 %>% dplyr::mutate(parent = NA)
    }

    df2
  })

  ds <- highcharter::list_parse(ds)
  ...
  highcharter::hc_add_series(highcharter::highchart(), data = ds, type = "treemap", ...)
})
...
hc_colorAxis(minColor =cols9[1],
  ...
  hc_title(text="Saldo balanza comercial e intercambio total, por zona económica y país. Acumulado 2026")%>%
...
Total comerciado, por país y continente - 2026 {data-navmenu="Comercio Exterior"}
=================================================================================


## Saldo balanza comercial e intercambio total, por continente y país. Acumulado 2026
...
spanish<-'{
	...
	"sLengthMenu":     "Mostrar _MENU_ filas - Acumulado 2026",
...
kable(continente,digits = 3 ,col.names=c("País / Continente","Exportaciones","Importaciones","Saldo","Comercio total"),caption="Saldo balanza comercial e intercambio total, por continente y país. Acumulado 2023") %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 22/8/2026.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2026/08/imig_julio_2026.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Julio 2026. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Julio 2026 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
...
Origen provincial de las exportaciones y complejos exportadores - 2026 {data-orientation=columns data-navmenu="Comercio Exterior"}
==================================================================================================================================
...
dta_clss<-regiones %>% 
  ...
  mutate(to = from + 500) %>% 
  mutate(color = cols14)%>% 
  mutate(name = paste0(name," : ",round(from,2), " mill. USD")) %>%
  list_parse()
...
hc_credits(enabled=TRUE, text="Año 2026.Fuente: INDEC", position =list( align ="left", x=20))
...
# en el segundo semestre de 2025 se excluyeró el desgloce por región  
#complejos<-complejos[-c(1,2,7,14,21,25,33),-2]
#
#complejos<-complejos %>% rename("Extranjero y plataforma continental"="Extranjero.y.plataforma.continental")%>%
...
#  hc_title(text="Complejos exportadores, desagregados por regiones económicas. Acumulado 1er. semestre 2025")%>%
#  hc_subtitle(text = "Datos estimados. Millones de dolares. Cliquear para conocer el detalle.") %>% 
#  hc_credits(enabled=TRUE, text= " Fuente: INDEC.  Dirección Nacional de Estadísticas del Sector Externo ",href = "https://www.indec.gob.ar/ftp/cuadros/economia/opex_anexo_cuadros_09_24.xls") %>%
#hc_tooltip(pointFormat = "<b>{point.name}</b>:<br>
#                             Exportaciones: {point.value:,.0f} millones de dólares")

#Quito lo que no sea complejo
complejos<-complejos[-c(1,2,8,13,20,25,33),]

complejos<-complejos %>% rename("Exportaciones"="Totales")

tm3 <- treemap(complejos, index = c("Complejos"),
              vSize = "Exportaciones",vColor="Exportaciones",
              type = "manual", palette = cols15,draw = FALSE)
...
hc_colorAxis(minColor =cols15[1],
               maxColor = cols15[20],enabled = TRUE)%>%
  hc_title(text="Complejos exportadores. Acumulado 2026")%>%
  hc_subtitle(text = "Datos estimados. Millones de dolares. Cliquear para conocer el detalle.") %>% 
  hc_credits(enabled=TRUE, text= " Fuente: INDEC.  Dirección Nacional de Estadísticas del Sector Externo ",href = "https://www.indec.gob.ar/ftp/cuadros/economia/complejos_exportadores_anexo_cuadros_26_08_26.xls") %>%
hc_tooltip(pointFormat = "<b>{point.name}</b>:<br>
                             Exportaciones: {point.value:,.0f} millones de dólares")
