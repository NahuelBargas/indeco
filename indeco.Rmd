...
basezonas<-read.xlsx("bases/basezonas29.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_octubre_2022.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
valueBox("-0.3 %", caption = "Variación del EMAE desestacionalizado de Septiembre-22 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("6.2 %", caption = "Variación acumulada del año(2022) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
### Balanza comercial, primeros diez meses de 2022
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros diez meses de 2022",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2022/11/resultado-fiscal_octubre-2022.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Octubre 2022.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Octubre 2022- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
