...
trimestre<-seq(as.Date("2016-04-01"), length = 39,by = "3 months")
...
basezonas<-read.xlsx("bases/basezonas65.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_junio26.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
regiones<-read.xlsx("bases/opex 2ndo semestre 2026.xlsx",sheet=1)

provincias<-jsonlite::fromJSON("bases/provincias.geojson", simplifyVector = FALSE)

complejos<-read.xlsx("bases/opex 2ndo semestre 2026.xlsx",sheet=2)
...
```{r}
valueBox("16.8 %", caption = "Acumulada en lo que va del año",  color="skyblue")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = "https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_07_26.xls")%>%		 
...
valueBox("-1.5 %", caption = "Variación del EMAE desestacionalizado de Abr.-26 con respecto al mes anterior", icon="glyphicon-arrow-down",color="yellow")
...
valueBox("2.1 %", caption = "Variación acumulada del año(2026) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="#43ac6a")#"color="yellow")
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros cinco meses de 2026",style = list( fontSize = '12px')) %>%
...
spanish<-'{
	...
	"sLengthMenu":     "Mostrar _MENU_ filas - Acumulado 2025",
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 20/6/2026.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2026/07/imig_junio_2026.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Junio 2026. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Junio 2026 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
...
Origen provincial de las exportaciones y complejos exportadores - 2025 {data-orientation=columns data-navmenu="Comercio Exterior"}
==================================================================================================================================
...
hc_credits(enabled=TRUE, text="Año 2025.Fuente: INDEC", position =list( align ="left", x=20))
...
complejos<-complejos[-c(1,2,8,14,21,24,33),]
...
hc_colorAxis(minColor =cols15[1],
               maxColor = cols15[20],enabled = TRUE)%>%
  hc_title(text="Complejos exportadores. Acumulado 2025")%>%
  hc_subtitle(text = "Datos estimados. Millones de dolares. Cliquear para conocer el detalle.") %>% 
  hc_credits(enabled=TRUE, text= " Fuente: INDEC.  Dirección Nacional de Estadísticas del Sector Externo ",href = "https://www.indec.gob.ar/ftp/cuadros/economia/opex_anexo_cuadros_09_24.xls") %>%
hc_tooltip(pointFormat = "<b>{point.name}</b>:<br>
                             Exportaciones: {point.value:,.0f} millones de dólares")
