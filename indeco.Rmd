...
trimestre<-seq(as.Date("2016-04-01"), length = 27,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 76)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 77, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 76)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas36.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_junio_2023.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
regiones<-read.xlsx("bases/opex 2ndo semestre 2022.xlsx",sheet=1)

provincias<-jsonlite::fromJSON("bases/provincias.geojson", simplifyVector = FALSE)

complejos<-read.xlsx("bases/opex 2ndo semestre 2022.xlsx",sheet=2)
...
```{r}
valueBox(paste(format(last(ipcmensual$IPC.General),digits=2),"%"), caption = paste0("Variación mensual del IPC - ",str_to_title(format(max(ipcmensual$Período),"%B-%Y"))), icon="glyphicon-arrow-down",color="skyblue")
```
...
```{r}
valueBox("50.7 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_07_23.xls")%>%		 
...
valueBox("-0.1 %", caption = "Variación del EMAE desestacionalizado de Mayo-23 con respecto al mes anterior",icon="glyphicon-arrow-down",color="skyblue")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, primer trimestre del 2023",style = list(color = "black", fontSize = '14px')) %>%
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Primer trimestre 2023. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
### Balanza comercial, primeros seis meses de 2023
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros seis meses de 2023",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2023/07/jun23.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Junio 2023.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Junio 2023- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
...
Origen provincial de las exportaciones y complejos exportadores - 2022 {data-orientation=columns data-navmenu="Comercio Exterior"}
==================================================================================================================================
...
hc_credits(enabled=TRUE, text="Segundo semestre 2022.Fuente: INDEC", position =list( align ="left", x=20))
...
hc_colorAxis(minColor =cols15[1],
  ...
  hc_title(text="Complejos exportadores, desagregados por regiones económicas. Acumulado 2ndo. semestre del 2022")%>%
