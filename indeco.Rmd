...
producto$Fecha <- as.yearqtr(2004 + seq(0, 84)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 85, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 84)/4),  "%q %Y")
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_julio2025.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
hc_subtitle(text="El VAB total asciende a 582.082 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Primer trimestre 2025. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros siete meses de 2025",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2025/08/sector_publico_base_caja_julio_2025.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Julio 2025. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Julio 2025 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
