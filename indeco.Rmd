...
producto$Fecha <- as.yearqtr(2004 + seq(0, 78)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 79, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 78)/4),  "%q %Y")
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_enero2024.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("36.6 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_03_24.xls")%>%		 
...
### Tasa de incidencia de la pobreza e indigencia- 1er Semestre 2023- Aglomerados
...
### Pobreza e indigencia-1er semestre 2023-Regiones
...
valueBox("-3.1 %", caption = "Variación del EMAE desestacionalizado de Diciembre-23 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("-1.6 %", caption = "Variación acumulada del año(2023) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, tercer trimestre del 2023",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 601421 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Tercer trimestre 2023. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primer mes de 2024",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2024/02/sector_publico_base_caja_enero_2024.xlsx")



IT<-tibble(name=c(basecaja[2,1],basecaja[12,1],basecaja[15,1],basecaja[19,1]),
y=c(basecaja[2,2],basecaja[12,2],basecaja[15,2],basecaja[19,1]),
drilldown = tolower(name))
...
hc_title(text="Resultado Fiscal- Base Caja-Enero 24.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Enero 2024- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
