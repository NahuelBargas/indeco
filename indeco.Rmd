...
producto$Fecha <- as.yearqtr(2004 + seq(0, 71)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 72, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 71)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas21.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado-fiscal_febrero_2022.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("16.1 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_04_22.xls")%>%		 
...
valueBox("0.5 %", caption = "Descenso del EMAE desestacionalizado de Enero-22 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("5.4 %", caption = "Variación acumulada del año(2022) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, cuarto trimestre del 2021",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 580109 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Cuarto trimestre 2021. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
### Balanza comercial, primer bimestre del 2022
...
impo <- tibble(
  ...
  y = c(10886,1912,4031,1264,2009,1323,251,94
...
y = c(6,211,35,122,72,2965,182,37,6,47,34,21
...
name = c("Carnes y sus preparados",
  ...
  "Lanas elaboradas",
  "Resto de MOA"),
y=c(586,33,226,8,9,24,134,939,42,98,129,1587,40,72,17,80
...
name= c(  "Productos químicos y conexos",
  ...
  "Resto de MOI"),
y= c(1045,186,28,4,75,21,1,22,404,289,207,706,3,43
...
y=c(144,189,1517,602,185,231,117,733,313
...
y=c(53,104,118,372,94,64,90,96,94,47,95,96
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primer bimestre del 2022",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 25/3/2022.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2022/03/resultado-fiscal_febrero_2022.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Febrero 2022.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Febrero 2022- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
