...
trimestre<-seq(as.Date("2016-04-01"), length = 32,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 81)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 82, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 81)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas48.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_agosto2024.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("94.8 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_09_24.xls")%>%		 
...
#polarcolumn
 aperturas_graf<- map(unique(aperturas$Aperturas), function(niveles){
  ...
  hc_credits(enabled = TRUE, text = "Fuente: INDEC",align = "right",verticalAlign = "bottom",
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_09_23.xls")%>%
    hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
})  
```
...
### Tasa de incidencia de la pobreza e indigencia- 2ndo Semestre 2023- Aglomerados
...
### Pobreza e indigencia-2ndo semestre 2023-Regiones
...
valueBox("-0.3 %", caption = "Variación del EMAE desestacionalizado de Mayo-24 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("-3.2 %", caption = "Variación acumulada del año(2024) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_subtitle(text="El VAB total asciende a 609083,6193 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Segundo trimestre 2024. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)

```

Balanza comercial, acumulado en 2024 {data-navmenu="Comercio Exterior"}
=========================================================================


### Balanza comercial, acumulado 2024
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros ocho meses de 2024",style = list( fontSize = '12px')) %>%
hc_subtitle(text="INDEC - Datos Estimados. Millones de dólares. Cliquear para desplegar.",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)

```

Saldo y comercio exterior total, por zona zona económica - 2024 {data-navmenu="Comercio Exterior"}
=================================================================================
...
hc_colorAxis(minColor =cols9[1],
  ...
  hc_title(text="Saldo balanza comercial e intercambio total, por zona económica y país. Acumulado 2024")%>%
...
Total comerciado, por país y continente - 2024 {data-navmenu="Comercio Exterior"}
=================================================================================


## Saldo balanza comercial e intercambio total, por continente y país. Acumulado 2024
...
spanish<-'{
	...
	"sLengthMenu":     "Mostrar _MENU_ filas - Acumulado 2024",
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 24/8/2024.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2024/09/sector_publico_base_caja_agosto_2024.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Agosto 24.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Agosto 2024- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
