...
trimestre<-seq(as.Date("2016-04-01"), length = 19,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 67)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 68, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 67)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas12.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_abril21.xlsx", sheet=2,startRow = 7,colNames=FALSE)
...
```{r}
valueBox("17.6 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_05_21.xls")%>%
...
### Tasa de incidencia de la pobreza e indigencia- 2ndo Semestre 2020- Aglomerados
...
row_spec(1, bold = T, color = "white", background = "lightseagreen")%>%
footnote("INDEC", symbol = "Debido a dificultades en el relevamiento de la información del tercer trimestre, se excluyen los datos del segundo semestre del aglomerado Ushuaia-Río Grande en el informe del INDEC.",general_title="Fuente: ",footnote_as_chunk = F, title_format = c("italic", "underline"),fixed_small_size= T)
...
### Pobreza e indigencia-2ndo semestre 2020-Regiones
...
valueBox("-0.2 %", caption = "Descenso del EMAE desestacionalizado de Marzo-21 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("2.4 %", caption = "Variación acumulada del año(2021) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, cuarto trimestre del 2020",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 537827 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
hc_exporting(enabled = TRUE)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Cuarto trimestre 2020. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
hc_exporting(enabled = TRUE)
...
### Balanza comercial, primer cuatrimestre del 2021
...
impo <- tibble(
  ...
  y = c(17549,3047, 7020, 1030, 3485, 2156, 655, 157 
...
y = c(10,474,73,182,180,3711,624,49,18,22,131,44
...
name = c("Carnes y sus preparados",
  ...
  "Lanas elaboradas",
  "Resto de MOA"),
y=c(1046,74,333,22,19,52,247,2627,77,200,280,4502,79,127,40,130
))
...
name= c(  "Productos químicos y conexos",
  ...
  "Resto de MOI"),
y= c(1183,244,60,8,130,35,2,46,653,402,402,1672,8,167
))
...
y=c(977,417,2415,901,250,305,167,1110,477
))
...
y=c(71,177,228,684,151,111,116,163,134,67,126,129))
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primer cuatrimestre del 2021",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 22/5/2021.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2021/05/resultado-fiscal-abril21.xls")
...
hc_title(text="Resultado Fiscal- Base Caja- Abril 2021.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Abril 2021- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
