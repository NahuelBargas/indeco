...
basezonas<-read.xlsx("bases/basezonas28.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_septiembre_2022.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("66.1 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_10_22.xls")%>%		 
...
valueBox("0.4 %", caption = "Variación del EMAE desestacionalizado de Agosto-22 con respecto al mes anterior", icon="glyphicon-arrow-up",color="skyblue")
...
valueBox("6.4 %", caption = "Variación acumulada del año(2022) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="orangered")
...
### Balanza comercial, primeros nueve meses de 2022
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros nueve meses de 2022",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2022/10/sector_publico_base_caja_-_sept-22.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Septiembre 2022.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Septiembre 2022- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
