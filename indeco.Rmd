...
basezonas<-read.xlsx("bases/basezonas18.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_noviembre21.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
### Balanza comercial, primeros once meses del 2021
...
impo <- tibble(
  ...
  y = c(56968,9013,23468,5135,10763,6497,1556,536
...
expo <- tibble(
  ...
  y = c(71320,20061,28629,18009,4620
...
y = c(22,1626,209,590,601,12326,3793,205,43,153,361,134
...
name = c("Carnes y sus preparados",
  ...
  "Lanas elaboradas",
  "Resto de MOA"),
y=c(3188,221,1026,62,120,146,785,8134,265,712,890,11876,243,404,114,441
...
name= c(  "Productos químicos y conexos",
  ...
  "Resto de MOI"),
y= c(4663,928,212,26,415,109,8,141,2186,1614,1259,5867,160,420
...
y=c(2796,1253,9449,2862,819,901,505,3455,1427
...
y=c(220,508,644,2084,468,300,405,473,387,229,404,377
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros once meses del 2021",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2021/12/resultado-nov21.xls")
...
hc_title(text="Resultado Fiscal- Base Caja- Noviembre 2021.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Noviembre 2021- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
