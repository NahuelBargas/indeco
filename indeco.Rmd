...
trimestre<-seq(as.Date("2016-04-01"), length = 18,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 66)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 67, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 66)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas8.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_noviembre_2020.xlsx", sheet=2,startRow = 7,colNames=FALSE)
...
```{r}
valueBox(paste(format(last(ipcmensual$IPC.General),digits=2),"%"), caption = paste0("Inflación del mes de ",str_to_title(format(max(ipcmensual$Período),"%B-%Y"))), icon="glyphicon-arrow-down",color="skyblue")
```
...
```{r}
valueBox("4 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_02_21.xls")%>%
...
Column {.tabset .tabset-fade style="width:85pc;height:80pc"}
--------------------------------------------------------------


### `r paste0("Variación mensual según aperturas seleccionadas","-",str_to_title(format(max(ipcmensual$Período),"%b-%y"))) `


 ```{r grafAper, }

#polarcolumn
  map(unique(aperturas$Aperturas), function(niveles){
  highchart() %>%
  ...
  hw_grid(rowheight = 260, ncol = 3)  %>% browsable()
...
### Tasa de incidencia de la pobreza e indigencia- 1er Semestre 2020- Aglomerados
...
### Pobreza e indigencia-1er semestre 2020-Regiones
...
scale_x_date(name="Trimestres",breaks=producto$fecha1,labels=producto$Trimestre,sec.axis = sec_axis(~.,labels = scales::date_format("%Y"),breaks=seq(as.Date("2004-06-01"), length = 18, by = "12 months")))+
theme_classic()+
...
valueBox("1.4 %", caption = "Aumento del EMAE desestacionalizado de Noviembre-20 con respecto al mes anterior", icon="glyphicon-arrow-up",color="skyblue")
...
valueBox("-10.6 %", caption = "Variación acumulada del año(Primeros once meses - 20) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, tercer trimestre del 2020",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 511236 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
hc_exporting(enabled = TRUE)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Tercer trimestre 2020. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
hc_exporting(enabled = TRUE)
...
### Balanza comercial, primeros once meses del 2020
...
impo <- tibble(
  ...
  y = c(38499,6571,15341,2542,6805,5487,1427,327),
  drilldown = tolower(name)
)
...
expo <- tibble(
  ...
  y = c(50996,15452,20665,11986,2893),
  drilldown = tolower(name)
)
...
y = c(15,1389,162,635,645,8603,3327,210,23,108,227,108) )
...
name = c("Carnes y sus preparados",
  ...
  "Lanas elaboradas",
  "Resto de MOA"),
y=c(3165,203,902,62,110,151,646,4659,214,678,810,8159,189,299,69,349))
...
name= c(  "Productos químicos y conexos",
  ...
  "Resto de MOI"),
y= c(2937,653,149,15,332,89,4,97,1871,1151,965,3394,76,253))
...
y=c(2134,460,6253,1968,744,589,322,1753,1117))
...
y=c(134,451,538,2019,265,407,278,394,231,147,317,304))
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros diez meses del 2020",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/resultado_fiscal_noviembre_2020.xls")
...
hc_title(text="Resultado Fiscal- Base Caja- Noviembre 2020.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja- Noviembre 2020- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
