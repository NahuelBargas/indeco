...
regiones<-read.xlsx("bases/opex 1er semestre 2023.xlsx",sheet=1)

provincias<-jsonlite::fromJSON("bases/provincias.geojson", simplifyVector = FALSE)

complejos<-read.xlsx("bases/opex 1er semestre 2023.xlsx",sheet=2)
...
```{r}
valueBox("20.6 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_02_24.xls")%>%		 
...
hc_credits(enabled=TRUE, text="1er. semestre 2023.Fuente: INDEC", position =list( align ="left", x=20))
...
complejos<-complejos[-c(1,2,7,14,17,24,32),-2]
...
hc_colorAxis(minColor =cols15[1],
  ...
  hc_title(text="Complejos exportadores, desagregados por regiones económicas. Acumulado 1er. semestre del 2023")%>%
