...
trimestre<-seq(as.Date("2016-04-01"), length = 35,by = "3 months")
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_junio2025.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("15.1 %", caption = "Acumulada en lo que va del año",  color="skyblue")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_07_25.xls")%>%		 
...
valueBox("-0.1 %", caption = "Variación del EMAE desestacionalizado de May-25 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
valueBox("6.1 %", caption = "Variación acumulada del año(2025) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-up",color="#43ac6a")
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros seis meses de 2025",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2025/07/sector_publico_base_caja_junio_2025.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Junio 2025. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Junio 2025 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
