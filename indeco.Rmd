...
basezonas<-read.xlsx("bases/basezonas30.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_noviembre_2022.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("85.3 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
valueBox("-0.3 %", caption = "Variación del EMAE desestacionalizado de Octubre-22 con respecto al mes anterior", icon="glyphicon-arrow-down",color="skyblue")
...
### Balanza comercial, primeros once meses de 2022
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros once meses de 2022",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2022/12/resultado-fiscal_noviembre-2022.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja- Noviembre 2022.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Noviembre 2022- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
