...
producto$Fecha <- as.yearqtr(2004 + seq(0, 80)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 81, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 80)/4),  "%q %Y")
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_julio2024.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
regiones<-read.xlsx("bases/opex 2ndo semestre 2023.xlsx",sheet=1)

provincias<-jsonlite::fromJSON("bases/provincias.geojson", simplifyVector = FALSE)

complejos<-read.xlsx("bases/opex 2ndo semestre 2023.xlsx",sheet=2)
...
```{r}
valueBox("87.0 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_08_24.xls")%>%		 
...
hc_subtitle(text="El VAB total asciende a 556084 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Primer trimestre 2024. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros siete meses de 2024",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2024/08/sector_publico_base_caja_julio_2024.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Julio 24.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Julio 2024- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
...
Origen provincial de las exportaciones y complejos exportadores - 2023 {data-orientation=columns data-navmenu="Comercio Exterior"}
==================================================================================================================================
...
hc_credits(enabled=TRUE, text="2ndo semestre 2023.Fuente: INDEC", position =list( align ="left", x=20))
...
hc_colorAxis(minColor =cols15[1],
  ...
  hc_title(text="Complejos exportadores, desagregados por regiones económicas. Acumulado 2ndo. semestre del 2023")%>%
  hc_subtitle(text = "Datos estimados. Millones de dolares. Cliquear para conocer el detalle.") %>% 
  hc_credits(enabled=TRUE, text= " Fuente: INDEC.  Dirección Nacional de Estadísticas del Sector Externo ",href = "https://www.indec.gob.ar/ftp/cuadros/economia/opex_anexo_cuadros_09_23.xls") %>%
hc_tooltip(pointFormat = "<b>{point.name}</b>:<br>
