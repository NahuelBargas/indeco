...
trimestre<-seq(as.Date("2016-04-01"), length = 16,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 64)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 65, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 64)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas4.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado-fiscal-julio_20.xlsx", sheet=2,startRow = 7,colNames=FALSE)
...
```{r}
valueBox("1.9 %", caption = "Inflación del mes de Julio-2020", icon="glyphicon-arrow-up",color="skyblue")
```
...
```{r}
valueBox("42.4 %", caption = "Variación interanual", icon="glyphicon-arrow-down",color="orangered")
```

### Acumulada en el año.

```{r}
valueBox("15.8 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = "https://www.indec.gob.ar/uploads/informesdeprensa/ipc_08_207BFCDEE221.pdf")%>%
...
### Variación interanual según divisiones
...
columns = c("GBA","Pampeana","Noreste","Noroeste","Cuyo","Patagonia"))),
rowStyle = function(index) {
  if (index == 1) list(fontWeight = "bold",color="white",background = "orangered")
})
...
### Tasa de incidencia de la pobreza e indigencia- 2ndo semestre 2019- Aglomerados


```{r preppob, include= FALSE  }
pobrezaaglo<-read.xlsx("bases/indicadores.xlsx", sheet=5,startRow=2,rowNames = TRUE)

pobrezaaglo[9,]<-c(rep(".",4))

pobrezaaglo_footnote <- pobrezaaglo

row.names(pobrezaaglo_footnote)[9] <- paste0(row.names(pobrezaaglo_footnote)[9], 
...
row.names(pobrezareg_footnote)[4] <- paste0(row.names(pobrezareg_footnote)[4], 
                                footnote_marker_symbol(1))
...
```{r tabla3}
kable(pobrezaaglo_footnote,escape = F) %>%
kable_styling(bootstrap_options = c("bordered","condensed"),full_width= FALSE) %>%
  add_header_above(c(" " = 1, "Pobreza(%)" = 2,"Indigencia(%)" = 2 ))%>%
row_spec(1, bold = T, color = "white", background = "lightseagreen")%>%
footnote("INDEC",symbol = "Debido a dificultades en el relevamiento de la información del tercer trimestre, se excluyen los datos del segundo semestre del aglomerado de Gran Resistencia en el informe del INDEC.",general_title="Fuente: ",footnote_as_chunk = F, title_format = c("italic", "underline"),fixed_small_size= T)
...
### Pobreza e indigencia-2ndo semestre 2019-Regiones

```{r tabla4}
kable(pobrezareg_footnote,escape = F) %>%
kable_styling(bootstrap_options = c("bordered","condensed"),full_width= FALSE) %>%
  add_header_above(c(" " = 1, "Pobreza(%)" = 2,"Indigencia(%)" = 2 ))%>%
row_spec(1, bold = T, color = "white", background = "orangered")%>%
footnote("INDEC", symbol = "Debido a dificultades en el relevamiento de la información del tercer trimestre, se excluyen los datos del segundo semestre del aglomerado de Gran Resistencia en el informe del INDEC.",general_title="Fuente: ",footnote_as_chunk = F, title_format = c("italic", "underline"),fixed_small_size= T)
...
png("tamaño completo.png", width=1024, height=483)
...
geom_line(aes(y=(PBI.hp$trend)/1000,color="PIB tendencia"))+
geom_point(aes(y=PIB.DESEST/1000),color=cols3[2])+
geom_point(aes(y=(PBI.hp$trend)/1000),color=cols3[3])+
geom_vline(xintercept=producto$fecha1[4],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[8],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[12],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[16],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[20],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[24],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[28],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[32],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[36],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[40],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[44],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[48],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[52],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[56],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[60],color="grey",size=0.1,linetype="dashed")+
geom_vline(xintercept=producto$fecha1[64],color="grey",size=0.1,linetype="dashed")+
scale_x_date(name="Trimestres",breaks=producto$fecha1,labels=producto$Trimestre,sec.axis = sec_axis(~.,labels = scales::date_format("%Y"),breaks=seq(as.Date("2004-06-01"), length = 17, by = "12 months")))+
theme_classic()+
...
valueBox("7.4 %", caption = "Aumento del EMAE desestacionalizado de Junio-20 con respecto al mes anterior", icon="glyphicon-arrow-up",color="skyblue")
...
valueBox("-12.9 %", caption = "Variación acumulada del año(Primeros seis meses - 20) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, primer trimestre del 2020",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 522,595 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
hc_exporting(enabled = TRUE)
...
hc_tooltip(pointFormat =  "{point.y} por ciento") %>%
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Primer trimestre 2020. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
hc_exporting(enabled = TRUE)

```

Balanza comercial, acumulado en 2020 {data-navmenu="Comercio Exterior"}
=========================================================================


### Balanza comercial, primeros siete meses del 2020
...
impo <- tibble(
  ...
  y = c(22718,3767,9084,1727,3921,3324,735,160),
  drilldown = tolower(name)
)
...
expo <- tibble(
  ...
  y = c(32291,10886,12479,6959,1967),
  drilldown = tolower(name)
)

balanza<-tibble(
name=c("Mercosur","Chile","Resto de ALADI","SICA","NAFTA","Unión Europea","Reino Unido",
...
y = c(23,845,89,320,507,5952,1621,127,43,64,214,74))
...
name = c("Carnes y sus preparados",
  ...
  "Lanas elaboradas",
  "Resto de MOA"),
y=c(1902,176,346,41,67,103,437,2816,137,628,491,5637,141,349,109,232))
...
name= c(  "Productos químicos y conexos",
  ...
  "Resto de MOI"),
y= c(2138,525,164,15,286,84,6,88,1441,1134,791,3660,84,160))
...
y=c(1374,264,3673,1177,464,340,179,951,663))
...
y=c(85,247,325,1257,145,300,179,237,111,90,171,179))
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros siete meses del 2020",style = list( fontSize = '12px')) %>%
hc_subtitle(text="INDEC - Datos Estimados. Millones de dólares. Cliquear para desplegar.",style = list(color = "#2b908f", fontSize = '10px')) %>%
hc_exporting(enabled = TRUE)

```

Saldo y comercio exterior total, por zona zona económica - 2020 {data-navmenu="Comercio Exterior"}
=================================================================================
...
hc_colorAxis(minColor =cols9[1],
  ...
  hc_title(text="Saldo balanza comercial e intercambio total, por zona económica y país. Acumulado 2020")%>%
...
Total comerciado, por país y continente - 2020 {data-navmenu="Comercio Exterior"}
=================================================================================


## Saldo balanza comercial e intercambio total, por continente y país. Acumulado 2020
...
continente[199,]<-rbind(c(0,colSums(continente[,2:5])))
continente[199,1]<-"Totales"

continente2<-arrange(basezonas, (Continente))
continente2<-continente2[,1:6]

continente2[199,]<-rbind(c(0,colSums(continente2[,2:5]),0))
continente2[199,1]<-"Totales"
continente2[199,6]<-"Totales"
...
spanish<-'{
	...
	"sLengthMenu":     "Mostrar _MENU_ filas - Acumulado 2020",
...
kable(continente,digits = 3 ,col.names=c("PaÍs / Continente","Exportaciones","Importaciones","Saldo","Comercio total"),caption="Saldo balanza comercial e intercambio total, por continente y país. Acumulado 2020") %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 26/8/2020.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/resultado-fiscal-julio_20.xls")
...
hc_title(text="Resultado Fiscal- Base Caja- Julio 2020.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja- Julio 2020- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
...
Origen provincial de las exportaciones y complejos exportadores - 2020 {data-orientation=columns data-navmenu="Comercio Exterior"}
==================================================================================================================================
