...
producto$Fecha <- as.yearqtr(2004 + seq(0, 87)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 88, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 87)/4),  "%q %Y")
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_abril26.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, cuarto del 2025",style = list(color = "black", fontSize = '14px')) %>%
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Cuarto trimestre 2025. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2026/05/cuentas_publicas3.rar")
...
hc_title(text="Resultado Fiscal- Base Caja-Abril 2026. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Abril 2026 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
