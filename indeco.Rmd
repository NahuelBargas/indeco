...
producto$Fecha <- as.yearqtr(2004 + seq(0, 77)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 78, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 77)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas40.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_septiembre_2023.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("103.2 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_10_23.xls")%>%		 
...
scale_x_date(name="Trimestres",breaks=producto$fecha1,labels=producto$Trimestre,sec.axis = sec_axis(~.,labels = scales::date_format("%Y"),breaks=seq(as.Date("2004-06-01"), length = 20, by = "12 months")))+
theme_classic()+
...
valueBox("2.4 %", caption = "Variación del EMAE desestacionalizado de Julio-23 con respecto al mes anterior",icon="glyphicon-arrow-up",color="skyblue")

#Fue quitado , icon="glyphicon-arrow-up"
```
...
valueBox("-1.8 %", caption = "Variación acumulada del año(2023) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, segundo trimestre del 2023",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 580780 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Segundo trimestre 2023. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)

```

Balanza comercial, acumulado en 2023 {data-navmenu="Comercio Exterior"}
=========================================================================


### Balanza comercial, primeros nueve meses de 2023
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros nueve meses de 2023",style = list( fontSize = '12px')) %>%
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2023/10/septiembre-2023.xls")
...
hc_title(text="Resultado Fiscal- Base Caja- Septiembre 2023.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Septiembre 2023- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_credits(enabled=TRUE, text="1er. trimestre 2023.Fuente: INDEC", position =list( align ="left", x=20))
...
hc_colorAxis(minColor =cols15[1],
  ...
  hc_title(text="Complejos exportadores, desagregados por regiones económicas. Acumulado 1er. trimestre del 2023")%>%
