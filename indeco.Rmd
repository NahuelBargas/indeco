...
producto$Fecha <- as.yearqtr(2004 + seq(0, 68)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 69, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 68)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas13.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_mayo21.xlsx", sheet=2,startRow = 7,colNames=FALSE)
...
```{r}
valueBox("21.5 %", caption = "Acumulada en lo que va del año",  color="yellow")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_06_21.xls")%>%
...
#polarcolumn
  ...
  hw_grid(rowheight = 130, ncol = 3)  %>% browsable()
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, primer trimestre del 2021",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 535009 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
hc_exporting(enabled = TRUE)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Primer trimestre 2021. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
hc_exporting(enabled = TRUE)
...
### Balanza comercial, primeros cinco meses del 2021
...
impo <- tibble(
  ...
  y = c(22690,3791,9134,1520,4501,2745,792,207
...
expo <- tibble(
  ...
  y = c(28314,7470,12635,6671,1538
...
y = c(13,586,97,230,260,4789,1162,52,23,29,172,56
...
name = c("Carnes y sus preparados",
  ...
  "Lanas elaboradas",
  "Resto de MOA"),
y=c(1344,94,419,28,29,67,311,3518,95,259,368,5623,101,165,46,170
...
name= c(  "Productos químicos y conexos",
  ...
  "Resto de MOI"),
y= c(1653,328,80,11,161,45,3,59,843,502,518,2204,73,191
...
y=c(1365,499,3184,1158,324,382,209,1414,599
...
y=c(90,217,293,894,187,141,151,207,167,84,155,158
))
...
hc_tooltip(pointFormat =  "{point.y} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primeros cinco meses del 2021",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 26/6/2021.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2021/06/sector_publico_base_caja_-_mayo_2021.xls")
...
hc_title(text="Resultado Fiscal- Base Caja- Mayo 2021.Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Mayo 2021- Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
