...
trimestre<-seq(as.Date("2016-04-01"), length = 34,by = "3 months")
...
producto$Fecha <- as.yearqtr(2004 + seq(0, 82)/4)
producto$Fecha <-format(producto$Fecha,  "%q %Y")
producto$fecha1<-seq(as.Date("2004-01-01"), length = 83, by = "3 months")
producto$fecha2<-format(as.yearqtr(2004 + seq(0, 82)/4),  "%q %Y")
...
basezonas<-read.xlsx("bases/basezonas53.xlsx", sheet=1)
...
## Resultado fiscal
basecaja<-read.xlsx("bases/resultado_fiscal_febrero2025.xlsx", sheet='IMIG',startRow = 7,colNames=FALSE)
...
```{r}
valueBox("4.7 %", caption = "Acumulada en lo que va del año",  color="skyblue")
```
...
highchart() %>% 
  ...
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_03_25.xls")%>%		 
...
#polarcolumn
 aperturas_graf<- map(unique(aperturas$Aperturas), function(niveles){
  ...
  hc_credits(enabled = TRUE, text = "Fuente: INDEC",align = "right",verticalAlign = "bottom",
  style = list(color = "#2b908f", fontSize = '10px'),
             href = " https://www.indec.gob.ar/ftp/cuadros/economia/sh_ipc_01_25.xls")%>%
    hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
})  
```
...
### Tasa de incidencia de la pobreza e indigencia- 1er Semestre 2024- Aglomerados
...
### Pobreza e indigencia-1er semestre 2024-Regiones
...
valueBox("0.5 %", caption = "Variación del EMAE desestacionalizado de Dic-24 con respecto al mes anterior", icon="glyphicon-arrow-up",color="skyblue")
...
valueBox("-1.8 %", caption = "Variación acumulada del año(2024) respecto al acumulado de igual periodo del año anterior", icon="glyphicon-arrow-down",color="orangered")
...
hc_colors(cols8)%>%
hc_title(text=" Valor agregado bruto a precios básicos,por categorías, primer trimestre del 2024",style = list(color = "black", fontSize = '14px')) %>%
hc_subtitle(text="El VAB total asciende a 589.270 millones de pesos y se obtiene de restarle al PIB los impuestos netos de subsidios, IVA y derechos de importación. <br> INDEC - Datos preliminares. Millones de pesos de 2004. ",style = list(color = "#2b908f", fontSize = '10px')) %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_subtitle(text="Fuente: INDEC - Datos preliminares. Tercer trimestre 2024. ",style = list(color = "#2b908f", fontSize = '10px'),align = "right") %>%
  hc_exporting(enabled = TRUE) %>% hc_add_theme(temaclásico)
...
hc_tooltip(pointFormat =  "{point.y:.1f} millones de dólares") %>%
  hc_colors(cols3)%>%
hc_title(text="Exportaciones por grandes rubros, importaciones por usos y agregados.Balanza comercial, por zonas económicas y países seleccionados. <br> Acumulado primer bimestre de 2025",style = list( fontSize = '12px')) %>%
...
- Estados Unidos incluye Puerto Rico y territorios asociados, China a Macao. Chile, Uruguay y Brasil, sus zonas francas.
- Última actualización: 22/3/2025.
...
hcrs <- highchart() %>%
  ...
  style = list(color = "#2b908f", fontSize = '8px'),href = "https://www.argentina.gob.ar/sites/default/files/2025/03/sector_publico_base_caja_febrero_2025.xlsx")
...
hc_title(text="Resultado Fiscal- Base Caja-Febrero 2025. Millones de pesos",style = list( fontSize = '13px')) %>%
...
hc_title(text="Resultado Fiscal- Base Caja-Febrero 2025 - Variaciones anuales en millones de pesos",style = list( fontSize = '13px')) %>%
